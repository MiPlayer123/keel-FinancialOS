#!/usr/bin/env bash
# Throwaway-Postgres validation for SLICE 6 statement ingest-begin
# (20260720220000_statement_ingest_begin.sql; statement-ingestion-v2.md §3/§4/§8/§12).
# Same pattern as run-statement-extraction-pgtap.sh: initdb a temporary cluster,
# build the minimal real schema from the test file, load the REAL shared helpers +
# the REAL Slice-0 + Slice-3 (both) + Slice-5 outbox + Slice-6 ingest-begin
# migrations, and run tests/pgtap/statement_ingest_begin.sql against the exact
# shipping SQL.
set -euo pipefail
...
MIG5="$ROOT/supabase/migrations/20260720210000_statement_outbox.sql"
MIG6="$ROOT/supabase/migrations/20260720220000_statement_ingest_begin.sql"
TESTSQL="$ROOT/tests/pgtap/statement_ingest_begin.sql"
...
# Render: inject helpers + slice0 + migrations at their markers.
RENDERED="$TMP/rendered.sql"
python3 - "$TESTSQL" "$HELPERS" "$SLICE0" "$MIG1" "$MIG2" "$MIG5" "$MIG6" "$RENDERED" <<'PY'
import sys
test, helpers, slice0, mig1, mig2, mig5, mig6, out = sys.argv[1:9]
src = open(test).read()
...
src = replace_line(src, '-- __MIGRATION5_BODY__', open(mig5).read())
src = replace_line(src, '-- __MIGRATION6_BODY__', open(mig6).read())
...
echo "=== running tests/pgtap/statement_ingest_begin.sql ==="
