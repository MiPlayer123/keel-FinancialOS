#!/usr/bin/env bash
# Throwaway-Postgres END-TO-END validation for the statement-ingestion pipeline
# (SLICE 10, statement-ingestion-v2.md §13) AND Issue #47 (single-source the
# approve-draft normalization). initdb a temporary cluster, build the minimal real
# schema from the test file, load the REAL shared helpers (sliced verbatim from
# their migrations) + the REAL migration chain in order, and run
# tests/pgtap/statement_pipeline_e2e.sql against the exact shipping SQL:
#   20260720100000 approval tokens (Slice 0)
#   20260720180000 statement extraction
#   20260720190000 statement anchor mode
...
#
# This is the frozen suite for the #47 refactor: it proves issue->approve of an
# unchanged body still succeeds (issue-hash == redeem-hash) and a tampered body is
# rejected, PLUS the whole pipeline's Law-5 red-team / Law-2 terminal-lock /
# Law-9 idempotency+strict-sum / A10 isolation / Law-6 export properties.
set -euo pipefail
...
MIG3="$ROOT/supabase/migrations/20260720250000_statement_issue_draft_approval.sql"
MIG4="$ROOT/supabase/migrations/20260720280000_statement_approve_draft_single_source.sql"
TESTSQL="$ROOT/tests/pgtap/statement_pipeline_e2e.sql"
...
# Slice the REAL shared helpers so the pipeline runs against the shipped plumbing.
...
# Render: inject helpers + slice0 + all four migrations at their markers.
RENDERED="$TMP/rendered.sql"
python3 - "$TESTSQL" "$HELPERS" "$SLICE0" "$MIG1" "$MIG2" "$MIG3" "$MIG4" "$RENDERED" <<'PY'
import sys
test, helpers, slice0, mig1, mig2, mig3, mig4, out = sys.argv[1:9]
src = open(test).read()
...
src = replace_line(src, '-- __MIGRATION3_BODY__', open(mig3).read())
src = replace_line(src, '-- __MIGRATION4_BODY__', open(mig4).read())
...
echo "=== running tests/pgtap/statement_pipeline_e2e.sql ==="
