#!/usr/bin/env bash
# Throwaway-Postgres validation for SLICE 9 statement holdings apply
# (20260720270000_statement_holdings_apply.sql; statement-ingestion-v2.md §5 [A8]).
# initdb a temporary cluster, build the minimal real schema from the test file,
# load the REAL shared helpers + the REAL Slice-0 (approval tokens) + Slice-9
# migration, and run tests/pgtap/statement_holdings_apply.sql against the exact
# shipping SQL.
set -euo pipefail
...
MIG9="$ROOT/supabase/migrations/20260720270000_statement_holdings_apply.sql"
TESTSQL="$ROOT/tests/pgtap/statement_holdings_apply.sql"
...
# Render: inject helpers + slice0 + slice9 at their markers.
RENDERED="$TMP/rendered.sql"
python3 - "$TESTSQL" "$HELPERS" "$SLICE0" "$MIG9" "$RENDERED" <<'PY'
import sys
test, helpers, slice0, mig9, out = sys.argv[1:6]
src = open(test).read()
...
src = replace_line(src, '-- __MIGRATION_BODY__', open(mig9).read())
...
echo "=== running tests/pgtap/statement_holdings_apply.sql ==="
