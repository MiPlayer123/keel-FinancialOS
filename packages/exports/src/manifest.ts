...
export const INCLUDE = [
  ...
  // Paycheck split templates SLICE B (paycheck-split-templates-v2.md §3 [AMENDED 6]).
  // Normalized template lines + the mutable per-series pointer/autonomy-grant +
  // class-B split suggestions. computed_components / ai_response jsonb are
  // ledger-derived (no secrets) → INCLUDED verbatim; the json-secret export scan
  // guards them (same reasoning as approval_tokens / category_suggestions).
  include({ schema: 'public', table: 'paycheck_template_lines', columns: ['household_id', 'id', 'template_id', 'line_key', 'kind', 'role', 'amount_kind', 'amount_minor', 'bps', 'category_ledger_account_id', 'destination_account_id', 'position', 'created_at'], sortKey: ['household_id', 'id'], timestampColumns: ['created_at'], bigintColumns: ['amount_minor'] }),
  include({ schema: 'public', table: 'paycheck_series_settings', columns: ['household_id', 'series_id', 'employer_id', 'active_template_id', 'booking_enabled', 'income_category_ledger_account_id', 'autonomy', 'updated_at'], sortKey: ['household_id', 'series_id'], timestampColumns: ['updated_at'], bigintColumns: [] }),
  include({ schema: 'public', table: 'paycheck_split_suggestions', columns: ['household_id', 'id', 'series_id', 'template_id', 'template_version', 'deposit_txn_id', 'computed_components', 'computed_gross_minor', 'computed_net_minor', 'source', 'ai_response', 'status', 'applied_paycheck_id', 'created_at', 'decided_at', 'decided_by'], sortKey: ['household_id', 'id'], timestampColumns: ['created_at', 'decided_at'], bigintColumns: ['computed_gross_minor', 'computed_net_minor'] }),
  // SLICE D: the audited apply record (token + suggestion + prior-offset undo
  // linkage). No secrets — a tenant-scoped link between an already-exported
  // paycheck/deposit/template and the token that approved it.
  include({ schema: 'public', table: 'paycheck_template_applications', columns: ['household_id', 'id', 'series_id', 'deposit_txn_id', 'template_id', 'template_version', 'paycheck_id', 'approval_token_id', 'applied_from_suggestion_id', 'prior_offset_ledger_account_id', 'prior_amount_minor', 'applied_by', 'revoked_at', 'revoked_by', 'created_at'], sortKey: ['household_id', 'id'], timestampColumns: ['revoked_at', 'created_at'], bigintColumns: ['prior_amount_minor'] }),
