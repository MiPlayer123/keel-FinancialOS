...
- **Owner ALTER is unguarded** (`alter … owner to keel_api;`), matching every sibling command migration (set_splits, paycheck templates) rather than `keel_finalize_link`'s guarded exception. Two CI failures found this the hard way: (1) `keel_finalize_link` (which I mirrored) uses `select … min(id)` on the uuid pk — **Postgres has no `min(uuid)` aggregate** (confirmed absent on live PG 17.6), so the proc raised `function min(uuid) does not exist` at runtime; rewrote as count-then-`limit 1` select. This is a genuine latent bug in `keel_finalize_link` too (likely the cause of the pre-existing `09-plaid-link` 500s in the integration job) — flagged for a separate fix, not touched here. (2) The guarded owner ALTER silently left the proc `postgres`-owned (pgTAP asserts `keel_api`): a bare-`postgres` apply can't reassign because **`keel_api` lacks CREATE on `public`** (`has_schema_privilege('keel_api','public','CREATE')=f` on live), which raises `permission denied for schema public`. CI's `supabase db reset` applier *can* reassign (018 proves `set_splits` ends up `keel_api`), so unguarded is correct there and is the house convention. The live apply of this migration must therefore run with a keel_api-capable role, exactly as the sibling commands were applied.
