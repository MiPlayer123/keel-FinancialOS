...
- **Categorization (§6, verified not assumed)**: the task cited a filename (`20260721070000_investment_offset_register_visibility.sql`) that does not exist in this repo. Traced the actual claim instead: `keel_list_transactions_rich_page`'s CURRENT latest body (`20260721020000_distribution_destination_register.sql`, superseding `20260718150000`/`20260720230000`/`20260721000000`) does carry `and ct.voided_at is null` (present twice — the header-pinned branch and the destination-register branch). Confirmed by direct grep of the live migration file, not taken on faith. No cleanup needed: a voided sweep candidate is already invisible in the register, and restoring it makes it reappear automatically once `voided_at` is cleared.
