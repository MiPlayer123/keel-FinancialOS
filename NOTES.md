...
## AI Agent — attach a receipt image to a transaction (Laws 2/5/7)
The agent can now file a chat-uploaded image as a receipt document on a transaction, reusing KEEL's existing document pipeline (`keel_documents_confirm_upload` with `p_target_type='transaction'`). NO new migration.
- **Scoped service-role capability**: the confirm proc + receipts bucket are service-role-only. The agent tool executor gets a NARROW injected closure `attachReceipt({transactionId})` (built in `/ai/chat`) — never `supabaseAdmin` directly. The closure verifies the txn is in the fixed household (user-client RLS query) → resolves its entity → `supabaseAdmin.storage.from('receipts').upload(householdId/docId/rand, bytes)` → `supabaseAdmin.rpc('keel_documents_confirm_upload', … p_target_type:'transaction')`. Household server-injected + re-verified by the proc (KEEL_SCOPE_VIOLATION); only the turn's uploaded image; receipt-compatible mimes only (JPEG/PNG/WebP/PDF; GIF rejected → capability absent). Class A auto+undo (undo = `documents.detach`, reusing `detachDocument`). authz `documents.attach_receipt` partner. 37 deno tests (+5: no-image, applies+detach-undo, capability-failure, authz-deny-blocks-capability). Prompt rule added + version `keel-agent@v4`.
- **REGRESSION FIXED (important)**: a concurrent fleet merge (`526164b`, "drop 5 stale authz test actions") had DROPPED my batch-2 `AGENT_WRITE_ACTIONS` (`transactions.categorize`, `categories.create`, `categories.rename`, `tasks.save`, `tasks.set_status`) from `packages/authz/src/action.ts` while the shipped agent tools still reference them — so on current main those tools would be **authz-denied at runtime** (broken). Restored all 5 (+ added `documents.attach_receipt`) to `AGENT_WRITE_ACTIONS`/`ACTION_MINIMUM_ROLES` and to the frozen `action.test.ts` vocabulary. authz 150 tests green. (The previously-deployed api at 15fea4e still had them; deploying current main WITHOUT this fix would have regressed recategorize/categories/tasks live.)
- **Scope note**: the agent attaches the user-uploaded image; it does not yet create a transaction from a receipt or run OCR extraction into fields (KEEL's separate receipt_extract pipeline). apps/web build green; ai 89 vitest.

## Agent tool-exposure architecture — research + slice 1 (dedupe read-tool proc dispatch)
Deep-research pass on how leading products expose product functionality to an LLM agent without an ever-growing hand-curated tool catalog (curated tools vs. MCP vs. code-execution-over-API/"Code Mode" vs. dynamic tool discovery), and where the safety guarantees have to live regardless of exposure mechanism. Verified finding that matters most for KEEL: code-then-execute (the Lightfield/Cloudflare "Code Mode" pattern) gives control-flow integrity (the model can only call what the sandbox/API authorizes) but explicitly NOT data-flow integrity (injected content can still corrupt what flows through an authorized call) — so suggest-then-approve + payload-binding approval tokens (Law 11) stay load-bearing no matter which exposure pattern is chosen; code execution is a way to REACH the API more efficiently, never a substitute for the human checkpoint on a write.

**Audit of KEEL's own catalog surfaced a real gap, not just an aesthetic one:** `packages/authz/src/action.ts` defines 112 authorized actions (`ACTIONS = WRITE_ACTIONS + READ_ACTIONS + AGENT_WRITE_ACTIONS`), but only 38 are wired into `supabase/functions/_shared/agent.ts`'s tool catalog — roughly two-thirds of what the domain layer already authorizes (e.g. `audit.read`, `balances.latest`, `documents.list_household`, `paychecks.templates`, `expected_reimbursements.list`, `admin.export_all`) is simply unreachable by the agent because nobody has hand-written a `ReadToolSpec`/`WriteToolSpec` entry for it. Follow-up investigation found the metadata needed to CLOSE that gap is asymmetric: the 63 `CommandName` write actions have real Zod payload schemas (`packages/contracts/src/commands.ts`) a JSON-Schema tool could be generated from; the 40 read actions + 10 `AGENT_WRITE_ACTIONS` have NO schema anywhere (parsed ad hoc per call site) — so closing the read-side gap needs new schema authoring (likely from the underlying SQL proc signatures), not just codegen. No risk-tier (A/B/C/D) lookup table exists as data either — only prose in this file.

**Slice 1 (shipped this pass — small, contained, no architecture bet):** the same investigation found proc dispatch duplicated in THREE independently hand-maintained places — `COMMAND_TO_PROC` and `QUERY_TO_PROC` in `supabase/functions/api/index.ts`, plus a third copy hand-typed directly into every `ReadToolSpec.proc` field in `agent.ts` — a live drift risk (if `QUERY_TO_PROC` changes and `agent.ts` isn't updated to match, the agent silently calls a stale/wrong proc). Fixed the read side (all 21 read tools had 1:1 identical `action`→`proc` pairs vs. `QUERY_TO_PROC`, confirmed by direct comparison): removed `ReadToolSpec.proc` entirely; `ReadToolDeps` now takes an injected `queryToProc: Readonly<Record<string,string>>` (the SAME `QUERY_TO_PROC` `index.ts` already uses for its own `/queries` dispatch — one source of truth, not two); `makeExecuteReadTool` resolves `queryToProc[spec.action]` and fails closed with `{error:'unmapped_tool'}` (never guesses/calls an undefined proc) if a mapping is ever missing. Added `READ_TOOL_ACTIONS` (tool name → action) as an export for future cross-checks. New regression test proves the fail-closed path. `index.ts`: exported `QUERY_TO_PROC`, wired `queryToProc: QUERY_TO_PROC` into the `makeExecuteAgentTool` deps.
- **Write side deliberately NOT touched this pass**: unlike reads, write-tool procs are woven into heterogeneous inline `execute()` closures (some call the real write proc, some call a read proc as a validation helper, e.g. `resolveCategoryName`/`resolveTaskRow` calling `keel_list_categories`/`keel_list_notes_tasks`), and only a subset of write actions even have a `COMMAND_TO_PROC` entry to dedupe against (the 10 `AGENT_WRITE_ACTIONS` — notes/tasks — aren't `CommandName` and have no entry at all). Deduping that safely needs its own per-tool review, not a mechanical pass. Flagged as follow-up.
- **Verification**: `deno test --allow-env supabase/functions/_shared/*.test.ts supabase/functions/worker/*.test.ts` → 75 passed (61 steps), 0 failed, including agent.test.ts's own 38 (37 existing + 1 new). Confirmed via `git stash` that pre-existing `tsc --noEmit` failures in `packages/exports` and `apps/web` (unrelated `exactOptionalPropertyTypes`/possibly-undefined errors) exist on the clean tree — not caused by this change. Could not run a full standalone `deno check` on `index.ts` in this sandbox (`npm:@supabase/server` isn't resolvable outside the Supabase deploy pipeline) — the 2-line diff there was verified by structural type-match against `AgentToolDeps` (fully exercised by the passing test suite) plus manual review, not a standalone compiler pass; worth a real `supabase functions deploy --dry-run`-equivalent check before/at next deploy.
- **Not yet decided**: whether to (a) generate write-tool schemas mechanically from `commands.ts` + `COMMAND_TO_PROC` to cut authoring cost for new `propose_*` tools, and (b) author the missing read/agent-write param schemas needed to close the 74-action reachability gap, and only then (c) decide code-execution vs. more hand-written tools for the read surface. Sequencing intentionally: (a)/(b) are prerequisites either way, per the research finding that code mode doesn't remove the need for a typed interface, it only changes how the model reaches one that has to exist first.

## Agent tool-exposure architecture — slice 2 (write-side proc dedup, CORRECTS prior note)
The prior entry's "write side deliberately NOT touched" reasoning was only half right — auditing every `WriteToolSpec.execute()` closure found the ACTUAL write-proc calls fall into two clean categories, not a messy pile: (1) the Class B `propose_*` tools (budgets, reimbursements, recategorize, categories) never call a write proc at all — they only stage a typed `proposed` payload; the real proc call happens later in the separate `/commands` envelope dispatch (already using `COMMAND_TO_PROC`, untouched). (2) The Class A auto-write tools (notes, tasks) DO call a write proc directly — `keel_note_save`/`keel_note_archive`/`keel_note_unarchive`/`keel_task_save`/`keel_task_set_status` — plus two inline READ-proc helper calls (`resolveCategoryName`→`keel_list_categories`, `resolveTaskRow`/`edit_note`→`keel_list_notes_tasks`) used only for validation/undo-capture.

**New finding, not caught in slice 1**: those 5 write-proc names were ALSO independently hardcoded in `api/index.ts`'s bespoke `/notes/save`, `/notes/archive`, `/notes/unarchive`, `/tasks/save`, and the status route — the exact same three-way-duplication pattern slice 1 fixed for reads, just missed because `AGENT_WRITE_ACTIONS` aren't `CommandName` so a `grep` against `COMMAND_TO_PROC` alone didn't surface it. Confirmed with the human before touching it, since it meant editing the LIVE note/task UI routes real users hit today, with no test file covering `index.ts`.

**Fix**: added `AGENT_WRITE_TO_PROC` (action → proc, 5 entries) next to `QUERY_TO_PROC` in `index.ts`; pointed the 5 bespoke UI routes at it (same string values, pure extraction, zero behavior change); merged it into the `queryToProc` map already injected into the agent (`{...QUERY_TO_PROC, ...AGENT_WRITE_TO_PROC}`). In `agent.ts`: `resolveCategoryName`/`resolveTaskRow` now take `queryToProc` and resolve `categories.list`/`notes_tasks.list` through it instead of hardcoding the read-proc name a third time; all 7 write-tool call sites (create_note, edit_note ×2, archive_note, restore_note, create_task, edit_task, set_task_status) resolve their proc through `ctx.queryToProc[action]` with the same fail-closed `unmapped_tool` guard as the read side, instead of a literal string. Net effect: every one of these 5 proc names now has exactly ONE production-code occurrence (in `AGENT_WRITE_TO_PROC`), down from three independent copies.
- New regression test: `propose_budget_target` (via `resolveCategoryName`) fails closed with `unmapped_tool` and never calls `rpc` when `categories.list` has no mapping — mirrors the read-side regression test from slice 1.
- **Verification**: full deno suite (`_shared` + `worker`) → 76 passed (61 steps), 0 failed (agent.test.ts: 39, +1 from slice 1's 38). `deno lint` clean on both changed files (no new findings vs. the pre-existing, unrelated `npm:` import-prefix + one unused-var warning already in `index.ts`). Manual re-grep confirmed zero remaining hardcoded occurrences of the 5 proc name literals outside `AGENT_WRITE_TO_PROC` and test fixtures/mocks.
- **Still not touched**: the Class B `propose_*` tools' eventual write-proc dispatch (the separate `/commands` envelope + `COMMAND_TO_PROC` path) — that's a different, already-single-sourced mechanism, out of scope here.

## AI Agent — conversation memory within one open chat (Law 5 unaffected)
Founder observation while testing (screenshot): asking "the stuff from the image?" or "do you have history?" got answered as if the conversation just started — the agent had genuinely zero memory of anything said earlier in the same visibly-open chat thread. Root cause, confirmed in code before touching anything: `apps/web/src/app/dashboard/assistant/page.tsx`'s `run({messages})` adapter called `latestUserQuestion(messages)` and sent ONLY that single latest question to `/ai/chat` — the file's own header comment even documented this as deliberate ("LocalRuntime keeps history; each run sends only the latest question"). `/ai/chat` and `runAgent` were fully stateless per request; no `history`/`conversationId` param existed anywhere, and no DB table for chat threads exists at all — this was never wired, not a regression.

**Scope decision (confirmed with the human)**: fix "remembers within one open chat" only — no new database table/migration. LocalRuntime already keeps the full visible thread client-side; the fix is purely "stop discarding it before the network call." Cross-session/reload persistence is a separate, bigger feature (would need a chat_messages/thread table) and was explicitly deferred.

**What changed**:
- `packages/ai/src/tool.ts`: `RunAgentInput` gains optional `history?: readonly HistoryTurn[]` (`{role:'user'|'assistant', text}`, oldest first). `runAgent` prepends it to the transcript before the new user message. Doc comment is explicit about why this doesn't reopen Law 5: the client only ever sees a turn's final tldr/body, never raw tool results, so no data-tier content is ever replayed through history — the spotlighting boundary still fully governs the CURRENT turn's own tool-use loop, untouched. Blank/whitespace-only history entries are dropped defensively. Exported `HistoryTurn` from the package. Confirmed via a new test that `openAiMessages`/`anthropicMessages` already serialize an arbitrary-length transcript generically (no assumption of a single prior user turn) — de-risked before touching anything. 3 new vitest (prepends history oldest-first; drops blank entries; a fresh conversation with no history is byte-identical to before). 92 vitest total (+3).
- `supabase/functions/api/index.ts` (`/ai/chat`): accepts optional `history` in the request body, validated + bounded SERVER-SIDE regardless of what the client sends (Law 12 discipline, same posture as the existing image-size cap) — max 20 turns, max 4000 chars/turn, max 12000 chars aggregate (oldest trimmed first past the aggregate cap). Malformed entries are dropped individually rather than failing the whole request. Passed through to `runAgent`. Telemetry gains `historyTurns: history.length` (a count only, never content — Law 12).
- `apps/web/src/lib/keel-api.ts`: `askKeel` gains optional `history?: AskKeelHistoryTurn[]`, sent only when non-empty.
- `apps/web/.../assistant/page.tsx`: new `conversationHistory(messages)` builds the prior-turns array from everything before the just-submitted question (client-side windowed to the same 20-turn cap as the server, to keep the request small), reusing a new shared `messageText()` helper (factored out of the existing `latestUserQuestion`). Corrected the file's own header comment, which had documented the stateless behavior as intentional.
- **Verification**: `pnpm --filter @keel/ai vitest run` → 92 passed. `deno test` full suite → 76 passed (61 steps), unchanged (this feature doesn't touch agent.ts). `cd apps/web && pnpm build` → clean (only pre-existing unrelated warnings in goals/import-csv-dialog/needs-attention). `deno lint` on `index.ts` → same 2 pre-existing unrelated findings as before, nothing new. Rebuilt the vendor bundle (`pnpm build:functions`) since `packages/ai` changed. Could not run a full standalone `deno check` on `index.ts` in this sandbox (same `npm:@supabase/server` resolution gap as prior slices) — mitigated this time by the fact that `apps/web`'s full `next build` typecheck DID cover the client-side half of this change end-to-end.
- **Not done**: durable persistence across reloads/devices (would need a `chat_messages`/thread table + a load-on-open fetch); a token/cost budget on top of the character caps (chars are a coarse proxy for tokens — fine for a first cut, worth revisiting if real usage shows it's too generous or too tight).

## AI Agent — close the reachability gap, batch 1 (16 new read tools)
Follow-up to the slice-1/2 proc-dedup audit's finding that only 38 of 111 authorized actions were agent-reachable. Regenerated the exact current gap (73 unreached) and triaged before writing any code, since this is a lot of new agent capability in a financial system — confirmed with the human ("do what is best for chat to work well") to proceed with the triage below rather than wire everything blindly.

**Two items in the raw gap list were red herrings, one is a hard blocker — all confirmed in code before acting**:
- `audit.read` has ZERO implementation anywhere (no proc, no route) — a registered action with nothing behind it yet, not an agent gap. Excluded; would need real backend work first.
- `transactions.list`/`transactions.rich` are older, superseded read shapes — the agent already has the best one (`transactions.rich_page`, as `list_transactions`). Excluded as pure redundancy.
- `paychecks.apply_template`/`paychecks.unapply` are **revoked from `service_role`/`anon` at the database level** (migration `20260722300000`: "A trusted agent principal can only SUGGEST"). Not a risk judgment call — a hard DB-enforced boundary already in place. Excluded from this and any future direct-call pass; if ever exposed, it'd have to be a Class B propose routed through the existing UI approval-token flow, never a new direct RPC.

**This batch — 16 read-only additions (agent.ts `READ_TOOL_SPECS`, 21 → 37 read tools)**: `list_recurring_classification`, `list_recurring_schedule_links`, `list_dismissed_paycheck_detections`, `list_paycheck_templates`, `list_paycheck_split_suggestions`, `list_expected_reimbursements` (a separate tracker from claim-based `reimbursements.*`, already covered), `list_statement_drafts`, `get_statement_cadence`, `suggest_statement_payments`, `get_statement_payment_links`, `get_statement_holdings_diff` (these 3 take a required `statementId`, validated as a uuid before ever reaching the proc), `list_documents_for_household`, `list_documents_for_target` (required `targetType` enum + `targetId`, both validated closed), `get_document_storage_summary`, `get_receipts_inbox`, `get_latest_balances`. All household-scoped via the existing `queryToProc` injection from slice 1 — zero new proc-dispatch duplication introduced. Proc signatures confirmed against the actual migrations before writing each `buildArgs` (all are plain `(p_household_id uuid)` except the 4 listed with extra params). Added friendly `TOOL_LABELS` entries in `apps/web/.../assistant/page.tsx` for provenance display (falls back to auto-prettified names otherwise, so this was polish, not required).
- New tests: catalog-presence check for all 16; a shared param-validation test across the 3 statement-scoped tools (missing/invalid `statementId` → `invalid_arguments`, proc never called); household-injection test for a statement-scoped tool; `targetType`/`targetId` validation for `list_documents_for_target`.
- **Verification**: full deno suite → 79 passed (61 steps), 0 failed (agent.test.ts: 42, +3). `packages/ai` unaffected (92 vitest, unchanged). `cd apps/web && pnpm build` → clean. Rebuilt the vendor bundle.
- **Deferred to a follow-up pass**: the ~19 write actions recommended as new Class B `propose_*` tools (`expected_reimbursements.*`, `recurring.{confirm,pause,resume,cancel,reject}`, `categorization.decide_suggestion`, `receipts.{decide_match,detach_match}`, `statements.{approve_draft,dismiss_draft,decide_payment_link,detach_payment_link,set_cadence}`, `paychecks.dismiss_detected`) — each needs its Zod payload schema in `commands.ts` inspected individually to build a correct proposal payload, more time-intensive and risk-sensitive than the read batch. The remaining ~32 write actions (raw ledger posting, account/connection lifecycle, reconciliation locks, `transactions.set_splits`, the full paycheck create/edit/apply lifecycle, document delete, etc.) are recommended to stay UI-only permanently — reasoning logged inline in the conversation, not repeated here; revisit this file's git history/chat log if the exclusion list needs re-litigating.

## AI Agent — close the reachability gap, batch 2 (18 new Class B propose_* write tools)
Follow-up to batch 1. Inspected every one of the 19 recommended write actions' Zod payload schema in `packages/contracts/src/commands.ts` before writing any tool — all 19 confirmed as real `CommandName` actions with a live schema (not `AGENT_WRITE_ACTIONS`), so each stages a `{kind, command, summary, payload}` proposal exactly like the shipped budgets/reimbursements tools, no new dispatch mechanism needed.

**One exclusion found mid-implementation, not in the original triage**: `statements.approve_draft` requires an `approvalTokenId` — it's TOKEN-BOUND (Law 11 hard gate), the same pattern as the already-blocked `paychecks.apply_template`. Staging a plain payload for it would either fail validation or (worse) misrepresent what approval means. Excluded from this batch; if ever wanted, it needs its own token-issuance route wired into the agent flow first, not a plain propose tool. `statements.dismiss_draft` (no token) shipped as planned.

**18 tools shipped** (agent.ts `WRITE_TOOL_SPECS`, 19 → 37 write tools): `propose_expected_reimbursement`, `propose_record_expected_reimbursement_receipt`, `propose_write_off_expected_reimbursement`, `propose_reopen_expected_reimbursement`, `propose_reverse_expected_reimbursement_receipt`, `propose_confirm_recurring`, `propose_pause_recurring`, `propose_resume_recurring`, `propose_cancel_recurring`, `propose_reject_recurring`, `propose_decide_category_suggestion`, `propose_decide_receipt_match`, `propose_detach_receipt_match`, `propose_dismiss_statement_draft`, `propose_decide_statement_payment_link`, `propose_detach_statement_payment_link`, `propose_set_statement_cadence`, `propose_dismiss_detected_paycheck`.

**Law 11 (approval summary must reflect the bound payload, not a model-supplied label) — two new resolvers, one deliberate exception**: added `resolveRecurringSeriesLabel` (→ `counterpartyKey` via `recurring.list`) and `resolveExpectedReimbursement`/`resolveExpectedReimbursementByReceipt` (→ `counterpartyName`/`description` via `expected_reimbursements.list`, the second finds the parent row owning a given `receiptId`), mirroring `resolveCategoryName` exactly — same fail-closed-on-unknown-id behavior, same `queryToProc` injection, zero new proc-dispatch duplication. Verified the real row shapes against the actual migrations first (`keel_list_recurring` exposes both `seriesId` AND `seriesKey` — confirmed the command payload needs `seriesId`, not the grouping key, before writing anything). The opaque-id "decide/detach/dismiss" cluster (categorization, receipts, statement payment-links/drafts, paycheck-dismiss) intentionally uses a generic summary with no resolved name — this matches the EXISTING precedent of the shipped `propose_reverse_reimbursement_claim`/`_settlement` tools, not a new gap: there's no name in those payloads for the model to mislabel in the first place.
- **UI wiring — verified, not written**: checked `ProposedActionCard`'s approve handler in `apps/web/.../assistant/page.tsx` before assuming anything works end-to-end. It only special-cases the 3 `AGENT_WRITE_ACTIONS` (`transactions.categorize`, `categories.create`, `categories.rename`); every other `command` already falls through to the generic `keelCommand` envelope call. Since all 18 new actions are real `CommandName`, they dispatch correctly with **zero UI changes** — confirmed by reading the code, not assumed.
- New tests: catalog-presence for all 18; expected-reimbursement resolver correctness + unknown-id rejection across all 4 resolver-dependent tools; recurring-lifecycle resolver correctness + `effectiveDate`/`horizonDays` defaulting + unknown-series rejection across all 6; opaque-id cluster arg validation + payload shape (incl. `set_statement_cadence`'s null-clears-cadence path); one authz-denial test.
- **Verification**: full deno suite → 85 passed (61 steps), 0 failed (agent.test.ts: 48, +6). `packages/ai` unaffected (92 vitest). `cd apps/web && pnpm build` → clean, no changes needed. Rebuilt the vendor bundle.
- **Batch 1 + batch 2 combined**: read tools 21→37, write tools 19→37, total agent tool catalog 40→74 tools, closing the reachability gap from 38/111 to **72/111** authorized actions (verified by direct comparison against `packages/authz/src/action.ts`, not estimated). The remaining 39 unreached — `audit.read` (no implementation), `ai_profile.save` (its own dedicated route), the 2 DB-revoked paycheck actions, the newly-found token-bound `statements.approve_draft`, the 3 read shapes superseded by `transactions.rich_page`/`list_transactions` (`transactions.list`, `transactions.rich`), and the ~32 deliberately UI-only mutations (raw ledger posting, account/connection lifecycle, reconciliation locks, the paycheck create/edit lifecycle, `transactions.set_splits`/`manual_create`/`manual_void`/`set_date`, document attach/detach/delete, statement create/holdings-apply, recurring schedule-linking) — are excluded for the reasons logged across both batch entries, not oversights.
## F-013 part 3/4 — Investment Flow Classifier (spec 2026-07-21-investment-flow-classifier-design)
Routes `keel_worker_ingest_investment_txn`'s offset BY FLOW instead of the sign-only uncategorized default, and SUPPRESSES core money-market sweeps. Fixes the live Fidelity symptoms: buys booked as spend, deposits/dividends as income, and the SPAXX core-sweep double-count that phantom-swung the register running balance and mis-detected as a same-account self-transfer.
- **Mapper** (`packages/providers/plaid/src/investments.ts`): `KeelPlaidInvestmentTxn.isCashEquivalent` = joined security `type === 'cash'` (false when no security, so pure cash deposits/withdrawals never flag). Worker passes it as `p_is_cash_equivalent`.
- **Migration `20260721060000`**: seeds per-entity `investments` (asset, is_category=false — the lever: `keel_cash_flow_monthly` sums only income/expense, so an asset offset is auto-excluded), `investment_income` (income), `investment_fees` (expense) via a new `keel_seed_investment_accounts` proc wired into the entity-insert trigger wrapper + backfilled for existing entities. Recreated the ingest proc with `p_is_cash_equivalent boolean default false`: buy/sell→investments asset, dividend_interest→investment_income, fee→investment_fees, else→uncategorized (unchanged). Core sweep (cash-equiv buy/sell OR description ILIKE '%CORE ACCOUNT%')→no canonical on fresh (`return null`), VOID (reversal + voided_at) on restatement of an existing row. **DROPPED the old 9-arg signature explicitly** — a default-valued param creates a SEPARATE 10-arg function, so without the drop both overloads exist and 9-positional-arg callers are ambiguous. Kept all idempotency/restatement/raw-versioning machinery. `keel_detect_transfers` restated verbatim from the 20260721010000 distribution pin with the same-account guard (`i.account_id <> o.account_id`) affirmed per tier (no behavioral change vs the pin; later timestamp so a fresh replay lands last).
- **Backfill moved to `scripts/backfills/investment_flow_backfill.sql` (NOT in supabase/migrations/)**: originally shipped as migration `20260721061000` in PR #149; a GitHub Codex review of the PR flagged (P1) that `supabase/migrations/` auto-applies via the Supabase GitHub integration on merge to main (`deploy-functions.yml` header comment), which would have run this live-data backfill un-reviewed the moment the PR merged. Relocated post-merge (follow-up commit) so it can only run via a deliberate manual `psql --single-transaction` per CLAUDE.md ops facts. Idempotent reprocess of existing `source='sync'` `economic_event_key like 'inv:%'` canonicals for ALL households: recovers flow from the source raw-event body's `flow` field; voids core sweeps (reversal + voided_at) and re-offsets buys/sells/dividends/fees via reversal + corrected batch (never UPDATE/DELETE postings). Clears only **`status='suggested'`** transfer_links referencing a voided sweep by DELETE (a suggested link has no decider, so removing it destroys no decision) — narrowed post-merge from an unconditional DELETE after both my local Codex review pass and the GitHub Codex PR review independently flagged it: a confirmed link can be a synthesized one-sided transfer (`keel_book_transfer_counterparty`, `booked_txn` pointing at a live counterparty batch) that a bare delete would orphan. Confirmed links on a voided sweep are left untouched and counted in a `confirmed_links_needing_review` notice for manual follow-up.
- **Post-merge Codex-review fixes** (follow-up commit, PR #149 already merged before these landed — caught in a second review pass after merge): (1) **sweep→real-flip bug**: a canonical previously voided by sweep-suppression has, by construction, no "live batch" (its original batch is reversed with a `journal_revisions` row whose `replacement_batch_id` is null) — restating it back to a real economic event unconditionally raised `KEEL_IMMUTABLE`, permanently blocking the un-void. Fixed: when no live batch exists AND the canonical's `voided_at` was already set, skip the reversal and post a fresh replacement batch directly (no `journal_revisions` link needed — the suppression's own revision row plus the voided_at→null flip is the audit trail). Regression-tested (sweep-flip test extended with the reverse direction). (2) **core-sweep false-positive**: the `description ILIKE '%CORE ACCOUNT%'` fallback was un-gated, so it could suppress a real deposit/withdrawal/transfer whose description merely named the core account — restricted the whole `v_is_core_sweep` check to `p_flow in ('buy','sell')` first. (3) **idempotency hash**: versioned on `flow` + `is_cash_equivalent` too (not just amount/date), so a provider restatement that changes only classification reclassifies instead of short-circuiting as an identical replay. (4) go-forward sweep-void path now also clears `status='suggested'` transfer_links on the voided canonical (parity with the backfill). (5) corrected a misleading comment claiming a `normalized_source_records` row is written for a suppressed sweep (it is not — no economic transaction exists to link one to).
- **Tests**: `supabase/tests/036_investment_flow_classifier.sql` (buy→investments asset & excluded from cash-flow; dividend→investment_income counted; fee→investment_fees; core sweep→no canonical, both cash-equiv and CORE ACCOUNT-described; real→sweep restatement→void; **sweep→real restatement→un-void, regression for the KEEL_IMMUTABLE bug**; deposit→sweep→buy balance conservation; same-account pair never linked). Updated `024_investments.sql` signature assertions to the 10-arg form. Mapper unit tests added (cash-equiv flagging on buy, real-equity not flagged, cash deposit not flagged). Did NOT apply migrations to the live DB — pgTAP not run locally (no Docker, and repo-wide CI is currently down — every job on main and every open PR fails at ~2s with zero steps executed, a GitHub Actions-level issue unrelated to this change); apps/web build + plaid vitest green; classifier migration additionally dry-run against the live cloud DB inside `BEGIN…ROLLBACK` (applies cleanly, seeds all 3 pfc_keys for all 4 real entities, correct signature) — committed nothing.
- **Independent review**: reviewed by both a local Codex pass (6-point focused review: postings-balance ✅, `keel_detect_transfers` byte-identical to the pinned definition ✅, idempotency-vs-double-post ✅, plus the 2 bugs and false-positive risk above) and Codex's automated GitHub PR review (same 2 bugs plus the migrations-auto-apply P1 and the synthesized-counterparty-leg P1 on the backfill DELETE). All findings addressed in this follow-up commit.

## Agent tool-exposure architecture — deployed, then diagnosed a real live failure (prompt v4 → v5)
Batches 1+2 (16 read tools, 18 write tools) plus the chat-history fix went live (PR #152 merged; `api` edge function redeployed to v83 from a local machine after this session flagged the 789KB-vendor-bundle context constraint on doing it via the Supabase MCP tool from here). Confirmed live via the founder's own transcript: cross-turn memory genuinely works now.

**Real failure found in the live transcript, diagnosed from the actual system prompt (not speculation)**: the founder gave the agent a full budget breakdown (income + 8 categories with percentages) and asked it to "insert this, a simplified version of this, into my budgeting tracker... you just propose something for me." The agent staged exactly ONE proposal (`budgets.set_expected_income`) while its own reply TEXT claimed it had staged two ("Total Budget: $44,200, Expected Income: $9,800... I have proposed the following") — a real narration-vs-tool-calls mismatch (the safety net held — nothing applied without approval either way, but the claim was false). The follow-up "I need to propose changes for everything" then got bounced back with "I need to know specifically what you want to change" — even though every number had just been given two messages earlier, now sitting in conversation history.

**Root cause, confirmed by reading `packages/ai/src/agent-prompt.ts` directly**: the system prompt had ZERO guidance for multi-step orchestration on compound requests — nothing telling the model to (a) call `list_categories` to resolve names before proposing per-category targets, (b) use `percentBp` directly for percentages instead of computing dollar amounts itself, (c) use numbers already given earlier in the conversation instead of re-asking, or (d) only describe a proposal as staged after actually calling its tool. This was a genuine prompt-engineering gap, not an inherent model limitation — confirmed the Anthropic default (`claude-sonnet-5`, api/index.ts:610) is already a current, strong model; the only model-quality risk is IF the OpenAI fallback (`gpt-4o-mini`, api/index.ts:616) is what's actually active live, which requires checking which provider key is configured — not visible from this session (a secret), flagged to the founder to confirm rather than guessed. Explicitly declined to hardcode a guessed model ID ("GPT 5.5") into the OpenAI fallback default — no confirmed knowledge such a model exists, and a wrong string would just fail at runtime.

**Fix — `AGENT_PROMPT_VERSION` bumped `keel-agent@v4` → `keel-agent@v5`**: extended the HARD RULES with a new rule 4 ("Be complete on multi-part requests") covering exactly the four gaps above, and added an explicit narration-integrity line to the writing-governance rule ("never summarize an intention as if it were an action"). Renumbered rules 5-7 accordingly. Kept it as tight, directive additions to the existing rule-list style rather than a rewrite — proportionate to the diagnosed failures, not a general prompt bloat. 2 new vitest assertions (multi-part completeness language present; narration-integrity language present). 94 vitest total (+2, tests file also fixed one self-inflicted line-wrap-boundary assertion bug). Rebuilt the vendor bundle; full deno suite unaffected (85 passed, this doesn't touch agent.ts/index.ts); `apps/web` build clean.
- **Open, unresolved**: which provider (Anthropic vs OpenAI) is actually active in the live deployment — needs the founder to confirm since it's a secret/env var, not something inspectable from a session. If OpenAI is active, `gpt-4o-mini` is a real, material capability ceiling for this kind of multi-step planning and is worth upgrading (to a real, confirmed model id — not guessed) or switching the deployment to Anthropic.

## Regular-sync core-account (SPAXX) sweep suppression — pair-gated, connection-id allowlisted (Laws 2/3/9/10)
Confirmed live bug distinct from (but the same CLASS as) F-013's investment-side core-sweep fix: the live Fidelity connection (`7e9bdccf-8fdc-4bdd-9805-5f3d4ac526e6`, household `a1ba3759-b7a7-4880-93e2-49eb6f91636c`) reports the SPAXX internal bookkeeping step ("PURCHASE INTO/REDEMPTION FROM/REINVESTMENT ... CORE ACCOUNT") AND the real external step ("TRANSFERRED TO VS Z37-...") as two SEPARATE, real `/transactions/sync` Plaid transactions (distinct ids, same date, exactly offsetting) — both post as real ledger rows on the SAME cash-management account, phantom-swinging the per-row running balance (confirmed: a mid-list -$24,951.76 balance on an account whose true balance is $48.24). No structured investment-type/cash-equivalent signal exists on this ingestion path (`packages/providers/plaid/src/adapter.ts`'s sync mapper carries only id/account/amount/date/description/pending), so this needed an independent, PAIR-GATED design — never a blanket description match.

- **Design correction #1 (connection_id, not institution_id)**: the design review's original draft scoped by `institution_id`, but the live Fidelity connection has `institution_id = NULL` in production (`finalize_link` only populates it when Plaid's Link flow supplies one) — an institution_id gate would have silently never fired. Implemented instead as `regular_core_sweep_allowlist` (household_id, connection_id) — a small, append-only, FK-tenant-scoped table. Adding a future connection is a one-row `INSERT`, no code change/redeploy. The migration seeds the ONE reviewed connection via `insert ... select ... where exists (...)` guarded on the connection actually existing in the target database — a no-op on fresh/local/CI databases, takes effect only once applied to the live cloud project where this connection is real.
- **Design correction #2 (regex anchor)**: the design doc's counterpart-description anchor was written `^TRANSFERRED (TO|FROM)\b`. PostgreSQL's Advanced Regular Expression dialect does NOT treat `\b` as a PCRE-style word-boundary metacharacter (`\y` is the ARE word-boundary atom; `\b` means backspace inside a bracket expression and is otherwise not a word-boundary escape) — using it as written would either error or silently fail to anchor. Corrected to `^TRANSFERRED (TO|FROM)([[:space:]]|$)`, the same anchor idiom already used for the sweep-pattern regex itself.
- **Migration `20260725000000_regular_core_sweep_suppression.sql`**: `regular_core_sweep_allowlist` (append-only, `keel_forbid_mutation`); `regular_core_sweep_suppressions` (provenance: household/connection/account, candidate+counterpart txn ids, their normalized-source-record ids, original/reversal/restore batch ids, prior_status, an evidence-hash fingerprint, command_id, `released_at`/`release_reason` — partial unique indexes enforce exactly one ACTIVE suppression per candidate and per counterpart). `keel_reconcile_regular_core_sweeps(household_id, connection_id)`: takes a `pg_advisory_xact_lock` per cash-management account under the connection (deterministic sort order, guards against the end-of-sync-attempt call racing the periodic refresh-balances call); candidate detection = `source='sync'` + `economic_event_key like 'txn:plaid:%'` + subtype `'cash management'` + start-anchored sweep regex + at least one linked `normalized_source_records.pfc_primary` of `TRANSFER_IN`/`TRANSFER_OUT` (the IMMUTABLE source signal, never the mutable `transaction_categories` overlay) + single-offset (2-posting) batch + not already actively suppressed; pairing = same account, EXACT same effective_date (no fuzzy window), exact equal-and-opposite cash-leg amount, counterpart not itself sweep-shaped, counterpart carrying transfer PFC or the corrected anchor regex; **strict degree-1 uniqueness on BOTH sides** via `count(*) over (partition by candidate/counterpart)` — an ambiguous many-to-many match suppresses NEITHER side, never guesses. Exclusions (confirmed/suggested transfer_links, `reconciliation_items`, locked `period_locks`, `transaction_overrides`, live `document_attachments`, split/multi-posting batches) are evaluated SEPARATELY from the base pattern match so the returned jsonb can report `candidatesConsidered` (base pattern, regardless of pairing/exclusion) and `excludedByPolicy` (the policy-guard subset) as distinct numbers — needed so the backfill script (which reuses this same RPC, never duplicating the predicate) can print an honest breakdown. Suppression mechanics mirror `keel_worker_ingest_investment_txn`'s core-sweep branch exactly: reverse the live batch (compensating batch, negated postings), `journal_revisions` row with `replacement_batch_id=null` (a void, not a replacement), `canonical_transactions.status='voided'`/`voided_at=now()`, clear only `status='suggested'` transfer_links the candidate participated in (a confirmed link is a real decision, left for manual review), `keel_finish_command`/`keel_idempotency_check` per suppressed pair for defense-in-depth idempotency on top of the advisory lock. A restored (released) suppression's evidence_hash is checked on every subsequent reconcile pass — the exact same pair is never immediately re-suppressed after a human restores it.
- **`keel_cmd_restore_regular_core_sweep(p_command_id, p_economic_event_key, p_actor, p_household_id, p_payload={suppression_id})`**: standard KEEL command-proc shape (mirrors `keel_cmd_reanchor_balance`) — `keel_assert_member_write` + `keel_actor_from_jwt` forgery guard + `keel_idempotency_check` + `keel_finish_command`, owned by `keel_api`. Re-posts the candidate's ORIGINAL postings (copied verbatim from the suppression's `original_batch_id`, not negated — that batch already holds the correct original signs) as a fresh live batch, un-voids the canonical (`status` back to the recorded `prior_status`, `voided_at=null`), and marks the suppression row `released_at`/`release_reason`. Deliberately does NOT insert a new `journal_revisions` link for the fresh batch — the suppression's own revision row (`replacement_batch_id=null`) already fully captures the history, and the `voided_at` null-flip plus the fresh live batch are the audit signal, exactly the precedent `20260721060000`'s sweep-to-real un-void path set. **Backend-only for v1, no UI button** (human decision — ship the correct, general command now; a UI can dispatch it later through the same `/api/commands` envelope with zero proc changes).
- **Companion restatement-safety review (verified against actual code, no unnecessary changes)**: read the CURRENT `packages/ingest/src/plan.ts` (not blindly trusting the design doc's citation) to determine whether `keel_worker_apply_action`/`keel_worker_lookup_state` need defensive changes so a row this new RPC voids isn't misreported to the sync planner as "provider removed." Found: **no change needed**. `planModified` already short-circuits to `noop`/`'already_applied'` whenever `prev.status === 'voided'` — BEFORE it would ever emit a `'revise'` action — so a later Plaid `'modified'` event on a voided candidate never reaches `keel_worker_apply_action`'s revise branch (the one that raises `KEEL_IMMUTABLE` on "no live batch to correct", `20260713090000_subcategories.sql:377-379`) at all. `planRemoved` similarly short-circuits a genuine provider-side removal of an already-voided row to a no-op, not a double-void. The one theoretical residual gap — a Plaid pending→posted MINT (a brand-new provider id whose `pendingTransactionId` points at the now-voided candidate) falls through `planAdded`'s supersession check (which only fires on `pending?.status === 'pending'`) into a plain `'create'` for the new id — is not a permanent double-count: the fresh row is simply a new, unsuppressed candidate that this SAME RPC's own next invocation (end-of-attempt or the periodic pass) re-evaluates and suppresses on its own merits, since voided rows are excluded from candidacy fresh each call. Documented in the migration's own header comment (§5) rather than patching working code "because the design doc said so."
- **Worker wiring** (`supabase/functions/worker/index.ts`): (a) end of every regular sync attempt (`processSyncNotification`) — added `household_id` to the connection select, call `keel_reconcile_regular_core_sweeps` right after the row-ingest action loop completes and before `completeSyncAttempt`/`keel_worker_complete_attempt`; best-effort (logs, never fails the attempt — the allowlist gate makes it a cheap no-op for every non-Fidelity connection). (b) the existing periodic refresh-balances cycle (`processRefreshBalances`) — added a recovery/catch-up loop over every active Plaid connection (no new cron job, reuses the existing 3-minute cycle), surfaced as `coreSweepsReconciled` in the response. Updated `supabase/functions/worker/test/index.test.ts`'s two RPC-mock admin objects that reach the full row-ingest path to handle the new RPC name (they previously threw "unexpected RPC" for anything unhandled) and added `household_id` to the mocked connection row.
- **Categorization (§6, verified not assumed)**: initially could not find `20260721070000_investment_offset_register_visibility.sql` because this work started from a worktree branch ~10 commits behind `origin/main` — that exact file lands in `762eaa2` ("restore register visibility for investment-asset offsets", merged after this worktree's base commit). Rebased onto `origin/main` before finishing (resolved one NOTES.md conflict, appended-after) and re-verified directly: `keel_list_transactions_rich_page`'s CURRENT latest body (`20260721070000_investment_offset_register_visibility.sql`, superseding `20260718150000`/`20260720230000`/`20260721000000`/`20260721020000`) does carry `and ct.voided_at is null`. Confirmed by direct grep of the live migration file, not taken on faith. No cleanup needed: a voided sweep candidate is already invisible in the register, and restoring it makes it reappear automatically once `voided_at` is cleared.
- **Tests**: `supabase/tests/037_regular_core_sweep_suppression.sql` (pgTAP, modeled on `036_investment_flow_classifier.sql`'s structure, plain INSERT/SELECT fixtures only — no helper functions/DO blocks, since pgTAP cannot be run locally here and I didn't want to risk an unproven procedural pattern in an unverifiable file). Covers: allowlist gate blocks before the connection is listed; (a) unambiguous pair suppressed (candidate voided + nets to zero, counterpart untouched); (b) unmatched sweep-pattern row never suppressed; (c) an ambiguous many-to-one match suppresses NEITHER side; (d) a candidate with a CONFIRMED transfer_links row is excluded; (e) a candidate in a locked period is excluded; the combined `candidatesConsidered`/`excludedByPolicy`/`ambiguousSkipped`/`suppressed` jsonb fields asserted exactly (6/2/2/1) via a captured temp-table result (asserting a live jsonb field via a bare re-invocation would re-run the non-replayed first call multiple times); (f) idempotent re-run — zero new suppressions, exactly one suppression row and one reversal batch after two calls; (g) restore un-voids + re-posts the original amount, and the SAME pair is not immediately re-suppressed by a follow-up reconcile (rejected-fingerprint guard); (h) tenant isolation — an identical-shaped pair in the Beta household is untouched by Alpha's reconcile call, but IS correctly suppressed when Beta's own (separately allowlisted) connection is reconciled.
- **Backfill**: `scripts/backfills/regular_core_sweep_backfill.sql` (NOT in `supabase/migrations/`, same auto-apply-avoidance rationale as `investment_flow_backfill.sql` — `deploy-functions.yml` auto-applies everything under `supabase/migrations/` on merge to main). Scoped to the ONE reviewed connection; refuses to run (raises) if that connection/allowlist row isn't present on the target database (a safety check against running against the wrong database, not silently no-op-ing). Reuses `keel_reconcile_regular_core_sweeps` directly — does not duplicate the candidate/pairing predicate — and prints a before/after `raise notice` summary (active-suppression count, voided-sync-transaction count, and the RPC's own `suppressed`/`candidatesConsidered`/`excludedByPolicy`/`ambiguousSkipped` fields). Idempotent by construction (the RPC's own advisory lock + "not already actively suppressed" filter). **NOT applied to live data** — this is a separate, deliberate manual step per the task's explicit instruction; the PR does not touch the live database.
- **Not run locally (documented limitation, not a blocker)**: pgTAP requires local Docker (unavailable here, consistent with every other slice in this journal). `deno test` for the worker suite also could not run in this sandboxed environment — `npm:@supabase/server@1.3.0` fails to resolve even after `pnpm install` (no network access to fetch it outside the pnpm-managed workspace deps), and this reproduces identically on the pre-existing, unmodified `main` branch (verified via `git stash` before restoring my changes) — a pre-existing environment limitation, not a regression from this change. Manually traced every RPC-mock in `index.test.ts` that reaches the new call site and added the missing case. Verified the migration's SQL by hand (paren/quote balance script, schema cross-reference against every touched table's actual column list) rather than by executing it. `apps/web` build is green (see below).

### Backfill fix — retroactive suppression needs a re-anchor too (found via a rolled-back live dry run on this PR, before merge)
Not speculation — a household-owner-run `BEGIN`/`ROLLBACK` dry run against the real cloud DB caught a genuine gap in `scripts/backfills/regular_core_sweep_backfill.sql` (the reconcile logic itself was untouched and confirmed correct): applying `keel_reconcile_regular_core_sweeps` to the real Fidelity "Individual" cash-management account (`abe2157a-08e2-4323-ba5c-da57278bb270`) correctly suppressed 6 pairs (phantom REDEMPTION/PURCHASE/REINVESTMENT legs voided, real TRANSFERRED legs correctly left live — Law 3 honored per-transaction throughout). But this account's opening-balance anchor was booked earlier via `keel_cmd_reanchor_balance`, reading Plaid's live `balance_snapshots.current_minor`, **before** this suppression existed — computed under the OLD history where each phantom+real pair already netted to zero on its own. Retroactively voiding only the phantom half of an ALREADY-anchored pair shifts Σ(postings) away from Plaid's true balance by the voided leg's full magnitude. Confirmed live, in one rolled-back transaction: before suppression the account correctly summed to $48.24; immediately after suppressing 6 pairs (before any re-anchor) it wrongly summed to **-$39,956.92**; re-running `keel_cmd_reanchor_balance` for the account brought it back to exactly $48.24.

- **Why this is a one-time-historical-backfill problem, not a go-forward one**: brand-new transactions landing via future syncs were never double-counted (no pre-existing anchor to conflict with, since the account's Σ(postings) already reflects the suppression from the moment each pair first lands). So neither `keel_reconcile_regular_core_sweeps` nor the worker call sites (end-of-sync, periodic refresh-balances) needed any change — only the standalone backfill script, which is reprocessing HISTORY that was anchored under the old (unsuppressed) numbers, needed the extra step.
- **Fix**: `regular_core_sweep_backfill.sql` now, as its final step, snapshots each account's ACTIVE-suppression count under the connection into a temp table (`tmp_core_sweep_reanchor_accounts`, `on commit drop`) BEFORE calling the reconcile RPC and again AFTER, then calls `keel_cmd_reanchor_balance` only for the account(s) where the count actually changed (`before <> after`) — never an account with zero new suppressions, so a repeat (already-suppressed) run stays a true no-op with no extra audit churn. Written generally (loops any account under the connection whose count changed, not just the one known Fidelity account), so a future connection with multiple cash-management accounts only re-anchors the ones this run actually touched.
- **Simulating the actor for `keel_cmd_reanchor_balance` from a bare psql script**: the proc derives its trusted actor from `request.jwt.claim.sub` via `keel_actor_from_jwt()` (ignores `p_actor`, a forgery guard) and `keel_assert_member_write` requires an `owner`/`partner` membership row — there is no real JWT outside an HTTP request. Looked up the household's `owner` (falling back to `partner`) membership and used `set_config('request.jwt.claim.sub', <user_id>, true)` (session-local) for the duration of each call — the exact precedent already set for the one-off Chase-checking manual re-anchor earlier in this file, not a new pattern.
- **Hard-fail, never swallow**: if no owner/partner membership exists for the household, the script itself now raises (refusing to leave a known-wrong ledger sum uncorrected) rather than skip the account silently. If `keel_cmd_reanchor_balance` raises for its own documented reason (no `balance_snapshots` row yet for the account — "refresh balances before re-anchoring"), there is deliberately no exception handler catching it: `ON_ERROR_STOP` + `--single-transaction` (already the script's invocation contract) abort and roll back the ENTIRE backfill, including the suppressions from the same run, rather than leave a half-applied state. Verified all of this end-to-end (fresh suppression + successful re-anchor; a second, idempotent no-suppressions run correctly re-anchoring nothing; the no-membership hard failure; and the `keel_cmd_reanchor_balance`-raises hard failure rolling back the whole transaction, including the suppressions inserted earlier in that same run) against a scratch local Postgres instance with a minimal stand-in schema and stubbed versions of the two RPCs — not against the live database, since this worktree cannot reach it; the orchestrating agent re-verified the real fix against the live cloud DB in its own rolled-back dry run.
- **Why `investment_flow_backfill.sql` did NOT need this and never will**: its suppression voids ONE self-contained 2-leg batch (the account's own leg + its offset leg) that always nets to zero entirely by itself — nothing else in the ledger was ever counting on that batch's magnitude, so voiding it cannot move Σ(postings) away from a previously-anchored target. This fix's suppression is asymmetric ACROSS TWO SEPARATE canonical transactions (the phantom candidate's batch is voided; the real counterpart's batch stays live) — Σ(postings) moves by the candidate's full magnitude, which is exactly the gap an anchor computed under the pre-suppression history needs corrected. The general rule for a future backfill: does the suppression's own batch net to zero by itself, or does it depend on a live counterpart transaction elsewhere in the ledger? Only the latter needs a re-anchor step.

### Apply-key bug fix — reconcile's idempotency key must vary with the (candidate, counterpart) PAIR, not just the candidate (independent Codex code review of this PR)
An independent Codex review of the migration (not something I'd have caught without it) found a real idempotency bug in `keel_reconcile_regular_core_sweeps`: `v_apply_key := 'core-sweep-suppress:' || r.candidate_txn::text` encoded ONLY the candidate transaction id, while `v_hash` (the same statement's `r.evidence_hash`, computed a few lines earlier from `jsonb_build_object('candidateTxnId', ..., 'counterpartTxnId', ...)`) encoded BOTH ids. `keel_cmd_restore_regular_core_sweep` legitimately un-voids a candidate so it can be re-evaluated by a later reconcile pass — and if that candidate then re-pairs with a DIFFERENT counterpart (e.g. the original counterpart got matched/superseded elsewhere and a new same-account/same-day/exact-opposite-amount transaction arrived), the new pairing's hash necessarily differs from the first pairing's hash, but the candidate-only key COLLIDES with the first pairing's already-recorded `command_executions` row. `keel_idempotency_check` would then raise `KEEL_IDEMPOTENCY_CONFLICT` — an UNCAUGHT exception inside the reconcile `for r in ... loop`, aborting the ENTIRE reconcile call (every other unrelated pair found in the same pass, too), not just that one candidate.
- **Fix**: `v_apply_key := 'core-sweep-suppress:' || r.candidate_txn::text || ':' || v_hash` — suffix the key with the full evidence_hash itself, rather than hand-rolling a `:v1:`-style version tag. Checked for an existing house convention first (grepped every `v_apply_key :=` in recent migrations): the investment ingest/classifier procs (`20260718121000`, `20260721060000`) already do exactly this — `'inv-ingest:' || provider_transaction_id || ':' || v_hash` — so this fix matches established idiom rather than inventing a new one. Appending the hash automatically versions the key on every field the hash covers (both transaction ids here) with no separate manual tag to keep in sync.
- **Verified the fix, not just asserted it**: extracted the edited function body and `CREATE OR REPLACE FUNCTION`'d it against a scratch local Postgres 17 instance with zero dependent tables — confirmed PL/pgSQL's lazy validation (embedded SQL in a function body is parsed for syntax only at creation, not resolved against the catalog until first call) means this is a legitimate syntax check even without stubbing the full schema. Separately unit-verified the array-dedup `= any(uuid[])` pattern used for the second fix below in an isolated `DO` block.
- **Two small hardening items from the same review, also fixed**: (1) `v_ambiguous_skipped` was incrementing once per ambiguous PAIR ROW, not once per distinct ambiguous CANDIDATE — a candidate with `candidate_degree > 1` (chasing more than one counterpart) produced multiple rows and over-counted relative to "how many candidates couldn't be resolved." Fixed with a `v_ambiguous_candidates uuid[]` dedup set checked via `= any(...)` before incrementing — a summary-accuracy-only change; the `continue` still fires for every ambiguous row, so WHICH pairs get suppressed vs skipped is unchanged. (2) Added a regression test to `supabase/tests/037_regular_core_sweep_suppression.sql`, scenario (i): reuses (a)'s already-suppressed-then-restored candidate `a1`, excludes its original counterpart `a2` from the pool via a legitimate CONFIRMED `transfer_links` row (mirrors (d)'s decoy pattern — needed so `a1` doesn't ambiguously match both `a2` and the new counterpart), introduces a brand-new counterpart `f1` (fresh dedicated id, the `f`-band — a literal `i` isn't valid hex) on the same account/day with the exact opposite amount, and asserts via `lives_ok` that the reconcile call SUCCEEDS (this is the exact call that raised `KEEL_IDEMPOTENCY_CONFLICT` before the fix), that `a1` is voided again against the NEW counterpart, that a NEW active suppression row is written, that `a1` now has two suppression rows total (the old released one + the new active one) with two DIFFERENT evidence hashes, and that the original counterpart `a2` stays untouched throughout.
- **Not run locally (same pre-existing limitation as the rest of this file)**: pgTAP requires local Docker, unavailable here. The orchestrating agent re-verifies against the live cloud DB in a rolled-back transaction before anything is applied for real.

## Plaid update mode (in-place re-authorization) — reissued-card silent-drop fix
Spec: `docs/superpowers/specs/2026-08-11-plaid-update-mode-design.md` (gitignored working doc). Diagnosis first: a live read-only investigation (BEGIN/rolled-back reads only) found the Chase Sapphire Reserve card (`…8311`, account `e92e11e8`) frozen since Jul 31 while its sibling checking (`…1223`) kept syncing on the SAME Plaid item — the item's `accounts` array dropped from 2→1 on Aug 2 (the reissued card fell out of the item). The item stays `active`, so KEEL's reauth prompt (only fires on `ITEM_LOGIN_REQUIRED`/`PENDING_EXPIRATION`, `plaid-sync.ts`) never surfaced. Root cause: KEEL had **no true Plaid update mode** — `linkTokenCreate` was never called with an existing item's `access_token`, so every "reconnect" minted a NEW item (duplicate-checking hazard + no clean reissued-card rediscovery).
- **Scope-changer discovered before building**: the ongoing sync path does NOT create accounts (`worker/index.ts` returns `unknown account` and drops the transaction) — accounts are only ever created at initial link via `keel_finalize_link`. So update-mode re-auth ALONE is insufficient; a reissued card Plaid re-surfaces would still be dropped. Update mode therefore ships paired with an account-reconcile step.
- **Backend**: two new `api` endpoints. `/connections/update-link-token` decrypts the connection's stored `access_token` (reuses the disconnect path's `keel_get_connection_credential_envelope` + `decryptToken`, factored into a new shared `decryptConnectionAccessToken` helper) and mints a link token via the new pure `buildUpdateModeLinkTokenBody` (`_shared/plaid-update.ts`) — passes `access_token` + `update.account_selection_enabled`, and NEVER `products` (Plaid rejects products+access_token; enforced by construction + unit-tested). `/connections/reconcile-accounts` calls `/accounts/get` → `mapAccountsGetToKeel` → new proc `keel_reconcile_connection_accounts`, then best-effort `keel_cron_drain_sync`. Both are `connections.link`-authorized + household-scoped.
- **Migration** `20260811120000_connection_reconcile_accounts.sql`: `keel_reconcile_connection_accounts(uuid,uuid,jsonb)` — INSERT-MISSING-ONLY (decision: add-missing-only; a transient Plaid omission must never archive a live account), mirrors `keel_finalize_link`'s account+ledger_account+account_owner creation and reuses `keel_resolve_finalize_entity` for per-account entity, writes `audit_log`, idempotent. Grant model mirrors `keel_cmd_set_splits` (owner keel_api, execute to `authenticated`, invoked over the user JWT so `keel_actor_from_jwt` resolves). Owner-change guarded like `keel_finalize_link`.
- **Frontend**: `createUpdateLinkToken`/`finishConnectionUpdate` in `keel-api.ts`; new `PlaidUpdateButton` ("Fix connection") on the Connections page per active connection (update mode returns no public_token — it does NOT exchange, it reconciles then syncs). Deliberately available even when `status==='active'` (the silent-drop state).
- **Out of scope (flagged)**: auto-detecting "an account that was in the item is now missing → prompt update" (the general fix for the silent-drop blind spot) and auto-archiving vanished accounts. Follow-ups.
- **Verification**: `pnpm typecheck` / `pnpm lint` (0 errors) / `pnpm test` (90 passed, 0 failed incl. 5 new `plaid-update.test.ts` cases) / `apps/web` build all green. Migration compile-checked against the LIVE schema in a `BEGIN … ROLLBACK` (all table/column/type refs valid, grants authenticated✓/anon✗, nothing persisted). pgTAP `supabase/tests/039_connection_reconcile_accounts.sql` (add-card / skip-existing / idempotent / entity / scope) runs in CI job 2 (`supabase test db`) — not runnable locally (no Docker, per every prior slice in this file). **Live migration apply + `api` function deploy are a separate, explicit human-confirmed step — NOT done by this PR.**
- **Owner assignment uses the temp-grant idiom** — `grant create on schema public to keel_api;` → `alter … owner to keel_api;` → `revoke create …` (documented in CLAUDE.md; copied verbatim from `20260723020000_recurring_reclassify_cadence.sql`). THREE CI rounds converged on this: (1) `keel_finalize_link` (which I mirrored) uses `select … min(id)` on the uuid pk — **Postgres has no `min(uuid)` aggregate** (confirmed absent on live PG 17.6), so the proc raised `function min(uuid) does not exist`; rewrote as count-then-`limit 1`. Genuine latent bug in `keel_finalize_link` too (likely the cause of the pre-existing `09-plaid-link` 500s in the integration job) — flagged, not touched here. (2) A guarded owner ALTER (mirroring `keel_finalize_link`) silently left the proc `postgres`-owned — pgTAP asserts `keel_api`. (3) A bare unguarded ALTER then aborted `supabase db reset` with `permission denied for schema public` — **`keel_api` lacks CREATE on `public`** at this point in the sequence (an earlier grant-tightening migration revoked it; that's why the FIRST command migrations' bare alters work but a LATE one like this fails). The temp-grant idiom fixes both. **Verified against the LIVE DB in a `BEGIN … ROLLBACK`**: after the migration the proc is `keel_api`-owned, `keel_api`'s CREATE is re-revoked (`f`), execute is authenticated✓/anon✗ — and it applies via the standard `postgres` connection, so the eventual live apply needs no special role.

## Market-value net worth + recurring editability + reimbursement picker (PR #165)
User-reported bug batch, every root cause verified against the live DB before any fix (four parallel read-only investigations + a live rolled-back repro).
- **Market-value net worth (decision)**: Plaid `balances.current` for a brokerage is the TOTAL account value (settlement cash + securities at market) — proven on live: `current − Σ holdings` is stable to the cent day-over-day while holdings move. The anchored cash ledger therefore (a) already contains securities value and (b) goes stale between anchors; the classifier's account-less `pfc_key='investments'` cost-basis ledger double-counted on top, and the sidebar (accounts-table-driven) never saw that account-less ledger — the exact $2,014.71 headline-vs-sidebar gap. Fix `20260813120000`: `keel_net_worth_as_of`/`keel_net_worth_daily` value investment-subtype accounts with a Plaid snapshot at the latest `balance_snapshots.current_minor` and exclude both ledger classes; manual investment accounts (no snapshot) keep ledger; non-investment accounts untouched; `keel_trial_balance` deliberately stays ledger-truth (register/reconciliation view). Frontend sidebar + Accounts page overlay the same per-account snapshot so all surfaces converge ($100,782.04 live). formulaVersion `net-worth-market-v1`/`net-worth-daily-market-v1`. Latent notes: `inv_value` hardcodes USD (live snapshots are all USD); daily-vs-as_of disagree for dates before an account's first snapshot (daily excludes the ledger from day one; as_of falls back to ledger until the first snapshot) — acceptable step-function semantics, revisit if pre-connection history matters.
- **CRITICAL perf regression caught by pre-merge adversarial review**: the first daily implementation ran ~42s on live (unbounded `days × inv` correlated subquery from 2021 + missing `household_id` predicate → the `(household_id, account_id, as_of DESC)` index couldn't serve it) and was killed by `authenticated`'s 8s statement_timeout — the dashboard trend was briefly broken on live. `20260813140000` bounds inv_daily to the output window, adds the household predicate, makes the bound sargable: 365-day window now ~0.2s under the authenticated role, identical output. Lesson recorded: EXPLAIN/timed-test read models under `set local role authenticated` before applying, not just as postgres.
- **Recurring "stuck" (UI-layer, spine was fine)**: 25 series reaper-`withdrawn` were invisible AND unactionable (read model filtered them; UI had no status). `20260813150000` removes `keel_list_recurring`'s withdrawn filter (consumers are status-driven — withdrawn/cancelled surface only in the new Stopped lane); web gains a Stopped section (withdrawn = "Stopped by KEEL", cancelled = "Stopped by you") with Restore (confirm-from-withdrawn/cancelled are legal transitions), a Stop-tracking control on the Paychecks detected card (recurring.cancel + bookingEnabled=false — cancel alone left the booking machine armed), and the client `changedToday` guard relaxed `>=`→`>` to match the server (same-day transitions allowed; the old guard hid all actions for a day). pgTAP 033 test 22 updated from "withdrawn not listed" to "listed with status withdrawn" (contract change, caught by CI).
- **Reimbursement "bugs out" (live rolled-back repro)**: tagging a PENDING txn (or transfer/distribution leg) fails `keel_live_real_posting`'s posted/reviewed+single-real-leg requirement → `KEEL_SCOPE_VIOLATION` → terse "Not found." toast. TxnPicker gains `eligibleOnly` (excludes pending + distribution legs) on all four reimbursement pickers + per-flow error copy.
- **Core-sweep incident + resolution (live ops, same day)**: applied `20260725000000` and called `keel_reconcile_regular_core_sweeps` DIRECTLY on historical data — reproducing exactly the failure this file already documents ("retroactive suppression needs a re-anchor"): cash-mgmt `abe2157a` went $48.24 → −$39,956.92. Emergency-restored all 6 via `keel_cmd_restore_regular_core_sweep`, then applied the CORRECT documented procedure: `scripts/backfills/regular_core_sweep_backfill.sql` (suppress + auto re-anchor) → 6 pairs voided, account re-anchored to Plaid's $48.38, idempotent no-op on re-run, allowlist re-seeded for go-forward worker suppression. **Deviation (user-approved 2026-08-13, hard-delete confirmation obtained)**: the emergency restore left (a) 6 released suppression rows whose fingerprint guard permanently blocked re-suppression and (b) 6 stale `command_executions` replay rows returning "suppressed" results for suppressions that had been reversed. Both sets — same-day artifacts of the botched direct call, not user decisions — were hard-deleted (the latter via a scoped disable/enable of `command_executions_immutable`); `audit_log`, `domain_events`, `journal_revisions`, and all batches from both cycles remain intact as the actual history. Rule reinforced: NEVER call the reconcile RPC directly on already-anchored history; the backfill script is the only sanctioned entry point.

## Category dropdown on the virtualized ledger scrolls the page to the top instead of opening (user report 2026-08-17)
Reproduced outside the app in a minimal harness pinning the exact shipped versions (@base-ui/react 1.6.0 + cmdk 1.1.1 + @tanstack/react-virtual 3.14.6, window virtualizer, popover+Command per row): clicking a trigger at scrollY≈44,000 produced ONE scroll event straight to the top, popup unmounted, focus on `<body>` — the exact user symptom. Instrumenting `scrollIntoView`/`focus` with stack traces caught the scroller: **cmdk's mount-time `scrollIntoView({block:'nearest'})` on its auto-selected first item/group heading**, fired during the first frame when the Base UI popup is parked at `position:absolute; top:0` of the DOCUMENT (rect y = −43,464). The window scrolls ~44,000px to reveal it; `useWindowVirtualizer` recomputes and unmounts the row that owns the `Popover.Root`; the popup is torn down in the same breath.
- **Why the July `nonModal` fix (ee077dd) didn't cure it**: the focus trap was never the (only) scroller. Verified in the same harness that `initialFocus={false}` ALSO doesn't fix it — the cmdk scroll fires regardless of any focus behavior.
- **Why Base UI's own guard fails**: `useAnchorPositioning` computes `{ position: 'fixed', ...originalFloatingStyles }` for the unpositioned frame, but Floating UI's `originalFloatingStyles` contains `position: 'absolute'` and is spread AFTER the fallback — the "default to fixed when not positioned to prevent autoFocus scroll jumps" comment in their source describes dead code (as of 1.6.0). Worth an upstream report.
- **Fix**: `positionMethod="fixed"` on the shared `PopoverContent` Positioner (`apps/web/src/components/ui/popover.tsx`) — a fixed-strategy popup is viewport-anchored in EVERY frame, so mount-time scrollIntoView/focus can never move the document (scroll chains of fixed elements end at the viewport). Applied at the shared primitive, not per-picker: any cmdk-in-popover (AddBudgetCategoryPicker too, and the Review-page picker on a scrolled page — same latent bug without the virtualizer masking it as "won't open") gets the fix. Matches Radix's popper default (`strategy: 'fixed'`), i.e. the behavior this UI had before the Base UI migration. Base UI runs `autoUpdate` while open, so the popup stays glued to its anchor when the page scrolls.
- **Verified in the harness**: with the fix, same click at the bottom → popup opens beside the trigger, survives, search input focused, typing filters; cmdk's two scrollIntoView calls hit rects already inside the viewport and move nothing. `nonModal` kept for its modality semantics (no trap/scroll-lock for a transient list popover); its doc comment corrected so the wrong causal claim doesn't mislead the next reader.
