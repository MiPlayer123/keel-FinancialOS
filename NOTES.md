...
- **Backfill `20260721061000` (NOT auto-applied — big REVIEW header)**: idempotent reprocess of existing `source='sync'` `economic_event_key like 'inv:%'` canonicals for ALL households. Recovers flow from the source raw-event body's `flow` field; voids core sweeps (reversal + voided_at) and re-offsets buys/sells/dividends/fees via reversal + corrected batch (never UPDATE/DELETE postings). Clears transfer_links referencing a voided sweep by DELETE — deviation from the soft-delete directive, justified: transfer_links is a DERIVED suggestion table (re-derivable by keel_detect_transfers), not canonical/secret truth, and its CHECK forbids setting a system-suggested link (decided_by null) to 'rejected' without a decider, so a schema-honest soft "reject" is impossible.
- **Tests**: `supabase/tests/036_investment_flow_classifier.sql` (buy→investments asset & excluded from cash-flow; dividend→investment_income counted; fee→investment_fees; core sweep→no canonical, both cash-equiv and CORE ACCOUNT-described; restatement→void; deposit→sweep→buy balance conservation; same-account pair never linked). Updated `024_investments.sql` signature assertions to the 10-arg form. Mapper unit tests added (cash-equiv flagging on buy, real-equity not flagged, cash deposit not flagged). Did NOT apply migrations to the live DB (per task) — pgTAP not run locally (no Docker); apps/web build + plaid vitest green.
