...
**Last updated:** 2026-07-12 (Stage 1D finance gates 6–8 GREEN)
...
**✅ GREEN: Stage 1D RECURRING (BC-v2.1 gate 5, uncommitted).** Pure `@keel/detectors` provides civil-date arithmetic, calendar-grid series detection, exact bigint statistics, confirmed-only projection, and deterministic timeline-aware backtesting. Detection persists immutable, evidence-bearing suggested candidate versions only; authenticated account-scoped commands bind to the locked candidate hash and audit confirm/pause/resume/cancel/reject. Worker/cron/metering, composite tenant integrity, RLS/ownership blocks, export coverage, pgTAP, and the end-to-end suggest→confirm proof are wired. Gate: 76 detector tests at 100% statements/branches/functions/lines; 388 Vitest + 12 Deno suites/55 steps; 248 pgTAP assertions; 92/92 integration across 12 files; C5c fetch-spy 0. No commit made.
**✅ GREEN: Stage 1D PAYCHECKS (BC-v2.1 mandatory gate 6, `e6bc05f`).** Typed immutable gross/addition/deduction/destination components reconcile gross, net, multiple direct deposits, 401(k)/HSA/FSA/ESPP/employer-match destinations, and tenant-owned live transaction capacity using bigint-only arithmetic. Manual/paystub/payroll source identity is replay-safe; duplicate source evidence and cross-paycheck over-allocation fail closed. Create/reverse/restore are authenticated, audited, idempotent, and append correction history; reads return exact strings, scope, as-of, formula version, source and transaction proof. Gate: workspace typecheck/lint/test green (410 Vitest + 12 Deno suites/55 steps); 297 pgTAP assertions; 97/97 integration across 13 files; C5c fetch-spy 0.
**✅ GREEN: Stage 1D P2P / REIMBURSEMENTS / BILL SPLITS (BC-v2.1 mandatory gate 7, pending commit).** First-class immutable expense shares and claim/settlement matches preserve the original expense, allocate only current tenant-owned live real-account transactions, reduce claims, and explicitly report zero income impact without writing new ledger postings. Create/settle/reverse commands are exact-bigint, idempotent, audited, reversible, cross-tenant fail-closed, and protected from concurrent expense/receipt over-allocation by transaction-scoped advisory locks. Read proof includes scope, as-of, formula version, remaining claim value, and settlement evidence.
**✅ GREEN: Stage 1D STATEMENTS / RECONCILIATION / CLOSE (BC-v2.1 mandatory gate 8, pending commit).** Immutable statement headers/lines reconcile opening + exact line sum = ending; close independently derives the same-currency ledger ending, requires one explanation per statement line and exact aggregate discrepancy explanations, emits append-only evidence/checklists, and installs a tenant/entity period lock. Explicit audited reopen reverses the lock before correction. Reads derive state without mutating source rows and return scope, as-of, formula version, line/item/adjustment/checklist proof. **Final shared gate for gates 7–8:** workspace 438 Vitest + 12 Deno suites/55 steps; clean-reset pgTAP 352 assertions; integration 104/104 across 15 files; C5c fetch-spy 0.
**✅ GREEN: Stage 1C REAUTH LIFECYCLE WIRING (uncommitted).** Live `/transactions/sync` non-2xx responses preserve only normalized `ITEM_LOGIN_REQUIRED` and `PENDING_EXPIRATION` as typed reauth signals; `ITEM_ERROR` is accepted only through its Plaid `error_type` paired with `error_code=ITEM_LOGIN_REQUIRED`. The worker owner-fenced abandons/releases before `keel_set_connection_reauth`, never advances cursor/freshness/canonical state, and leaves generic transient errors retry-only. Only verified `ITEM` webhooks may select the fixed lifecycle path: `ITEM_LOGIN_REQUIRED`, `PENDING_EXPIRATION`, `PENDING_DISCONNECT`, `USER_PERMISSION_REVOKED`, and `ITEM_BAD_STATE` set reauth; `LOGIN_REPAIRED` clears it; transaction/default codes retain the existing record+enqueue path. Hermetic integration proves no lifecycle webhook sync enqueue, invalid signatures cannot act, the acquire guard refuses reauth connections, canonical rows stay unchanged, and fetch-spy remains 0. Gate: 297 Vitest + 12 Deno suites/55 steps; 200 pgTAP; 88/88 integration across 11 files. Interactive update-mode remains deferred to 1E per PLAN-1C §3.
**AFTER 1D recurring:** independent post-build review + human-approved commit; then remaining Stage-1C deploy-⚑ (KEK rotation route, Plaid Dashboard webhook, secret cron schedules, production), remaining Stage 1D finance domains (paycheck/statements), and Stage 1E viewer UI. Deferred export work: async Storage/job path, step-up MFA, operational cross-project restore, import→canonical lineage.
