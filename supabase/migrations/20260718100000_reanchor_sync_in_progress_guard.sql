-- Production bug (2026-07-18): a household owner's newly-linked Chase
-- checking account (554 transactions, ~4 minutes to fully backfill) showed
-- a balance $2,632.76 off from Plaid. Root cause traced via balance_snapshots
-- + journal_batches history: keel_cmd_reanchor_balance ("Fix balance")
-- computes opening = provider_balance - Σ(journal_postings AS OF RIGHT NOW),
-- which is a moving target while the worker is still landing backfill pages.
-- The user (reasonably, given no feedback the sync was still running)
-- clicked Fix balance 12 times over 34 seconds while the backfill was still
-- in flight; each click captured a different partial transaction set, and
-- the LAST click landed ~3 minutes before the sync actually finished
-- (confirmed via connections.last_successful_sync_at), so its "correction"
-- went stale as soon as the remaining transactions landed, with nothing to
-- auto-correct it afterward. The credit card on the same connection (only
-- 50 transactions, fast backfill) was unaffected -- same class of race,
-- just never lost it.
--
-- Fix: reject the command outright while the account's connection has an
-- outstanding sync generation (sync_desired_generation <>
-- sync_committed_generation covers "actively leased right now" AND
-- "queued but not yet picked up" AND "partially committed"). This protects
-- every caller (web UI, future API/MCP clients), not just this button --
-- Law 7. The existing keel_apply_account_balance one-time auto-anchor
-- already deferred on last_successful_sync_at; this closes the same class
-- of gap for the user-triggered manual correction, which had no such guard.
...
begin
  ...
  -- Sync-in-progress guard: see migration header. Σ(journal_postings) below
  -- is only trustworthy once the connection's backfill has fully committed.
  if v_account.connection_id is not null then
    ...
    if v_conn_desired_gen is distinct from v_conn_committed_gen then
      raise exception
