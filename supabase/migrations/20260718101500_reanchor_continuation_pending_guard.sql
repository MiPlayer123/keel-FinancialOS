-- Independent codex review on PR #60 caught a real gap in the
-- sync-in-progress guard added there: sync_committed_generation is bumped
-- to match sync_desired_generation on EVERY attempt completion, including a
-- PARTIAL page (p_fully_synced=false) whose continuation hasn't run yet --
-- completeSyncAttempt (supabase/functions/worker/sync-completion.ts:40-58)
-- calls keel_worker_complete_attempt first, then enqueues the continuation
-- as a SEPARATE step, and the continuation reuses the SAME generation
-- number (keel_worker_open_attempt always opens at the connection's
-- CURRENT sync_desired_generation). So for the whole window between "this
-- page committed" and "the continuation's own attempt completes", desired
-- == committed even though the backfill is still mid-flight -- reopening
-- the exact balance-corruption path PR #60 was meant to close, on any
-- multi-page initial sync (not just this specific incident's timeline,
-- where the gap happened to be covered by an unrelated new-generation
-- bump).
--
-- Fix: track "a continuation is expected" explicitly rather than inferring
-- it from generation numbers, which don't carry that information. New
-- connections.sync_continuation_pending is set atomically inside
-- keel_worker_complete_attempt (same transaction, so no ordering/fencing
-- gap versus a separate follow-up call) whenever the completing page is
-- 'partial'; cleared on 'terminal' or 'noop'. keel_cmd_reanchor_balance
-- now also rejects while it's set.
--
-- Time-bounded, not permanent: a connection whose continuation job never
-- got picked up (worker crash, permanent reauth failure, etc.) would
-- otherwise stay flagged forever, breaking the documented recovery path
-- for stuck syncs ("Fix balance" is the manual escape hatch when a
-- connection never completes -- see reanchor_balance.sql's own header).
-- sync_continuation_marked_at bounds the block to 15 minutes (5x the
-- ~3-minute cron drain cadence), well past any healthy continuation but
-- well short of "the user is stuck waiting indefinitely."
alter table public.connections
  add column if not exists sync_continuation_pending boolean not null default false,
  add column if not exists sync_continuation_marked_at timestamptz;

drop function public.keel_worker_complete_attempt(uuid, uuid, text, boolean);
...
create function public.keel_worker_complete_attempt(
  ...
  p_continuation_pending boolean default false
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  update public.connections
     set sync_committed_generation = v_attempt.generation,
         ...
         sync_continuation_pending = p_continuation_pending,
         sync_continuation_marked_at = case
           when p_continuation_pending then now()
           else null
         end,
...
alter function public.keel_worker_complete_attempt(uuid, uuid, text, boolean, boolean) owner to keel_worker;
revoke all on function public.keel_worker_complete_attempt(uuid, uuid, text, boolean, boolean) from public, anon, authenticated;
grant execute on function public.keel_worker_complete_attempt(uuid, uuid, text, boolean, boolean) to service_role;
revoke create on schema public from keel_worker;

-- keel_cmd_reanchor_balance: add the continuation-pending check alongside
-- the generation-match check from PR #60 (kept -- it still correctly
-- catches an outright NEW desired generation, e.g. a fresh "Sync now").
...
begin
  ...
  -- 20260718101500). Σ(journal_postings) below is only trustworthy once
  -- the connection's backfill has fully committed with no continuation
  -- still expected.
  if v_account.connection_id is not null then
    select sync_desired_generation, sync_committed_generation,
           sync_continuation_pending, sync_continuation_marked_at
      into v_conn_desired_gen, v_conn_committed_gen,
           v_conn_continuation_pending, v_conn_continuation_marked_at
      from public.connections
    ...
    if v_conn_desired_gen is distinct from v_conn_committed_gen
       or (v_conn_continuation_pending
           and v_conn_continuation_marked_at > now() - interval '15 minutes') then
      raise exception
