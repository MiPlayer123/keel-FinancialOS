...
create function keel_assert_member_write(p_household_id uuid) returns household_role
language plpgsql
security definer
...
create function keel_idempotency_check(
  p_household_id uuid,
  p_key text,
...
as $$
declare
  v_existing command_executions%rowtype;
begin
...
create function keel_payload_hash(p_payload jsonb) returns text
...
create function keel_finish_command(
  p_command_id uuid,
  p_command text,
  p_key text,
  p_household_id uuid,
...
begin
  ...
  insert into domain_events (event_type, household_id, command_id, economic_event_key, actor, payload)
...
create function keel_insert_postings(p_batch_id uuid, p_postings jsonb) returns jsonb
language plpgsql
security definer
set search_path = public
as $$
...
-- ---------------------------------------------------------------------------
create function keel_cmd_create_account(
  p_command_id uuid,
...
as $$
declare
  v_hash text := keel_payload_hash(p_payload);
  v_replay jsonb;
  ...
  v_result jsonb;
begin
  perform keel_assert_member_write(p_household_id);
  v_replay := keel_idempotency_check(p_household_id, p_economic_event_key, v_hash);
  if v_replay is not null then
    return v_replay;
  end if;

  if not exists (
    select 1 from entities e where e.id = v_entity_id and e.household_id = p_household_id
  ) then
    raise exception 'KEEL_SCOPE_VIOLATION: entity % not in household', v_entity_id
      using errcode = 'P0006';
  end if;

  insert into ledger_accounts (household_id, entity_id, name, kind, currency, is_category)
  values (
    p_household_id, v_entity_id, p_payload->>'name',
    (p_payload->>'kind')::ledger_account_kind, p_payload->>'currency', false
  )
  ...
  values (v_account_id, (select auth.uid()));
...
-- ---------------------------------------------------------------------------
create function keel_cmd_record_raw_event(
  p_command_id uuid,
  p_economic_event_key text,
  p_actor jsonb,
  p_household_id uuid,
...
begin
  ...
  select c.id into v_connection_id
   ...
   where c.household_id = p_household_id
     and c.provider = (p_payload->>'provider')::bank_provider
  ...
  select r.id into v_raw_id
   ...
   where r.connection_id = v_connection_id
     and r.provider = (p_payload->>'provider')::bank_provider
  ...
  if v_raw_id is null then
    ...
    values (
      ...
      (p_payload->>'provider')::bank_provider,
...
-- ---------------------------------------------------------------------------
create function keel_cmd_post_batch(
  p_command_id uuid,
  p_economic_event_key text,
  p_actor jsonb,
  p_household_id uuid,
...
begin
  ...
  v_postings := keel_insert_postings(v_batch_id, p_payload->'postings');
  ...
  perform keel_enqueue('transaction_enrichment', jsonb_build_object(
...
-- ---------------------------------------------------------------------------
create function keel_cmd_promote_event(
  p_command_id uuid,
  p_economic_event_key text,
  p_actor jsonb,
  p_household_id uuid,
...
begin
  ...
  values (
    ...
    (p_payload->>'status')::transaction_status,
    (p_payload->>'source')::transaction_source,
    p_payload->>'description',
    (p_payload->>'effective_date')::date,
...
-- ---------------------------------------------------------------------------
create function keel_cmd_reverse_batch(
  p_command_id uuid,
  p_economic_event_key text,
  p_actor jsonb,
  p_household_id uuid,
...
declare
  v_hash text := keel_payload_hash(p_payload);
  v_replay jsonb;
  v_original journal_batches%rowtype;
  v_reversal_id uuid;
  v_result jsonb;
begin
  perform keel_assert_member_write(p_household_id);
  ...
  if exists (
    select 1 from journal_revisions r
  ...
  insert into journal_revisions (original_batch_id, reversal_batch_id, reason)
...
-- ---------------------------------------------------------------------------
create function keel_trial_balance(p_household_id uuid) returns jsonb
language plpgsql
security definer
set search_path = public
stable
...
begin
  ...
  select coalesce(jsonb_agg(row order by row->>'ledgerAccountId'), '[]'::jsonb) into v_rows
    from (
      ...
      from journal_postings p
      join journal_batches b on b.id = p.batch_id
...
create function keel_list_transactions(p_household_id uuid) returns jsonb
language plpgsql
security definer
set search_path = public
stable
...
begin
  foreach f in array array[
    'keel_assert_member_write(uuid)',
    'keel_idempotency_check(uuid, text, text)',
    'keel_finish_command(uuid, text, text, uuid, jsonb, text, text, text, uuid, jsonb, jsonb)',
    'keel_insert_postings(uuid, jsonb)',
    'keel_cmd_create_account(uuid, text, jsonb, uuid, jsonb)',
    'keel_cmd_record_raw_event(uuid, text, jsonb, uuid, jsonb)',
    'keel_cmd_post_batch(uuid, text, jsonb, uuid, jsonb)',
    'keel_cmd_promote_event(uuid, text, jsonb, uuid, jsonb)',
    'keel_cmd_reverse_batch(uuid, text, jsonb, uuid, jsonb)',
    'keel_trial_balance(uuid)',
    'keel_list_transactions(uuid)'
  ] loop
...
revoke all on function keel_assert_member_write(uuid) from authenticated;
revoke all on function keel_idempotency_check(uuid, text, text) from authenticated;
revoke all on function keel_finish_command(uuid, text, text, uuid, jsonb, text, text, text, uuid, jsonb, jsonb) from authenticated;
revoke all on function keel_insert_postings(uuid, jsonb) from authenticated;

grant execute on function keel_cmd_create_account(uuid, text, jsonb, uuid, jsonb) to authenticated;
grant execute on function keel_cmd_record_raw_event(uuid, text, jsonb, uuid, jsonb) to authenticated;
grant execute on function keel_cmd_post_batch(uuid, text, jsonb, uuid, jsonb) to authenticated;
grant execute on function keel_cmd_promote_event(uuid, text, jsonb, uuid, jsonb) to authenticated;
grant execute on function keel_cmd_reverse_batch(uuid, text, jsonb, uuid, jsonb) to authenticated;
grant execute on function keel_trial_balance(uuid) to authenticated;
grant execute on function keel_list_transactions(uuid) to authenticated;
