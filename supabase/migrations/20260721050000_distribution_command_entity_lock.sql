-- 20260721050000_distribution_command_entity_lock.sql
--
-- Distribution follow-up (Codex review Findings: stale entity + TOCTOU).
--  (1) set_splits validated account/category legs against v_cash.entity_id,
--      taken from the HISTORICAL journal posting — stale after
--      keel_reassign_account_entity. Now the cash entity is read from the
--      CURRENT owning account row (a.entity_id), the source of truth.
--  (2) Both commands read the target account/ledger without a lock before
--      keel_insert_postings; a concurrent archive/reassign could slip in.
--      Added `for update of a` on the owning-account and account-leg lookups
--      so the account row is pinned through the batch insert.
-- Bodies otherwise identical to the live 20260721000000 definitions.
...
begin
  ...
  select a.id, a.entity_id, a.ledger_account_id, a.archived_at,
    ...
    for update of a;   -- lock owning account (TOCTOU)
  if not found then
