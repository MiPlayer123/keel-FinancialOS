-- 20260720230000_set_splits_balanced_mixed.sql
-- Allow BALANCED mixed-direction splits (paycheck decomposition: gross income
-- − tax expenses − 401k transfer = net). Corrects the set_splits direction
-- guard to validate each leg against its own offset sign rather than the whole
-- transaction's cash sign. Balance + entity + live-category + currency checks
-- unchanged. Enables transaction-level paycheck splits without corrupting the
-- kind-classified read models. Idempotent CREATE OR REPLACE.
...
begin
  ...
  for v_split in select * from jsonb_array_elements(p_payload->'splits') loop
    ...
    -- net). The original guard (r3603509629) checked every leg against the whole
    -- transaction's cash sign, which forbade any mixed entry. The corruption it
    -- guarded against — an income share summed as a credit on a cash-out — is
    -- fully prevented by validating each leg against ITS OWN offset sign
    -- instead: an expense offset is positive (debit), an income offset is
    -- negative (credit). Balance is still enforced (splits sum to -cash), and
    -- the split-aware read models classify each offset by its own kind, so a
    -- balanced mixed split reports correctly (gross→income, taxes→expense). A
    -- genuinely misassigned leg (income category on a positive/expense-side
    -- offset, or vice versa) still rejects here.
