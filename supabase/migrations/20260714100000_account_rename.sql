...
begin
  ...
  -- Codex review (PR #9, P2): every real account has a backing ledger_accounts
  -- row created with the same name, and exports/chart-of-accounts consumers
  -- (e.g. packages/exports/src/beancount.ts) derive the account name from
  -- THAT column, not accounts.name — rename both in the same transaction or
  -- the old name lingers everywhere but the account page itself.
  update public.ledger_accounts set name = v_name where id = v_row.ledger_account_id;
...
-- Same gap on ledger_accounts (also select+insert only, per the same
-- migration): needed for the backing-ledger-account rename above. RLS's
-- `ledger_accounts_definer_all` policy (generated by the dynamic loop over
-- table names in that same migration) already permits the row.
grant update (name) on public.ledger_accounts to keel_api;
