-- Confirmed transfers vanish into a bare "Transfer" badge once approved: the
-- Review page shows both account names during confirm, but the ledger row
-- and edit dialog show neither afterward (FEATURE-GAP follow-up). This is
-- pure read-model plumbing (Law 1 n/a — no arithmetic): join the confirmed
-- transfer_links row to its OTHER leg's cash account and surface that
-- account's id/name so the UI can render "Transfer -> Chase Savings"
-- instead of "Transfer". Additive only: existing columns are untouched.
--
-- Full recreate of the tags-aware body (20260713120000) with one added
-- lateral for the counterparty leg's account.
