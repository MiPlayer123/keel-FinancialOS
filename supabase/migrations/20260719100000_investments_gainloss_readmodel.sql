...
-- portfolio totals on the Investments tab.
--
-- Pure function-body replace of keel_investments_overview(uuid): the signature
-- is BYTE-IDENTICAL to 20260718122000_investments_read_models.sql and grants /
-- policies / ownership are untouched. holdings.cost_basis_minor is already
-- stored and already emitted per-holding as costBasisMinor; v1 adds derived
-- unrealized gain/loss (per holding + USD aggregates) with NO schema change and
-- NO ledger interaction (holdings never post).
--
-- Reproducible numbers (Law 9): asOf + formulaVersion on the payload, and the
-- with-basis subset is surfaced (holdingsWithBasisCount / holdingsCount) so an
-- excluded partial-basis portfolio can never masquerade as a complete gain.
-- Money = BIGINT minor units, no floats (Law 4): every money/ratio is emitted
-- as TEXT (bigint never rides a JSON number); a missing cost basis stays JSON
-- null and is never coerced to 0. unrealizedGainBps is an integer basis-point
-- ratio (round(...)::bigint) that guards BOTH null basis and divide-by-zero.
...
begin
  ...
  return jsonb_build_object(
    ...
    'formulaVersion', 'investments-overview-v2',
    'accounts', v_accounts,
    'holdings', v_holdings,
