-- Fourth-round codex finding on PR #60, P1: the sync-in-progress guard
-- (20260718100000, 20260718101500) reads sync_desired_generation /
-- sync_committed_generation / sync_continuation_pending with a plain
-- SELECT, once, near the top of the function. Both keel_worker_bump_generation
-- and keel_worker_acquire_sync_lease commit their state changes via a plain
-- UPDATE on the SAME connections row. Between the guard's read and the
-- function's later Σ(journal_postings) computation (after the currency
-- check, two period-lock checks, and the reversal loop), a concurrent sync
-- could start -- pass fine at the guard's instant, then post new
-- transactions before this transaction's own SUM reads them -- reproducing
-- the exact race this whole PR exists to close, just through a narrower
-- window.
--
-- Fix: SELECT ... FOR NO KEY UPDATE on the connections row at the same
-- point the guard already reads it (same convention keel_worker_complete_
-- attempt already uses locking its own connections row). Both
-- keel_worker_bump_generation's and keel_worker_acquire_sync_lease's plain
-- UPDATEs will block behind this lock until the reanchor transaction
-- commits or rolls back, so no concurrent sync can transition the
-- connection's generation/lease state -- and therefore can't start posting
-- new transactions -- for the remainder of this function's execution.
-- FOR NO KEY UPDATE (not the stronger FOR UPDATE) avoids blocking
-- unrelated FK-referencing inserts against this row.
...
begin
  ...
  -- 20260718101500, 20260718103500). FOR NO KEY UPDATE holds this row
  -- lock through the rest of the transaction so no concurrent sync can
  -- transition generation/lease state (and therefore can't start posting
  -- new transactions) while Σ(journal_postings) below is computed.
